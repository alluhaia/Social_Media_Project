...
* @ return array $result : the array of stats array   ( date , source_id , profile_id , total, average,
								active_day, unique_author, popular_author, positive, negative, neutral, retweets,
								percent_retweets, total_urls, percent_urls,top_language, profile_twitter_attention
								,top_authority, top_authority_followers, total_leads ) 
*
*/
...
*/
	...
	public function insertStats ($array) {
				...
				$item['date']=explode(" ",date('Y-m-d H:i:s')); // get only the date from datetime
				...
				if (is_null($raw['date'])==true) { 
						...
						$query ='INSERT INTO twitter_stats 
							( date , source_id , profile_id , total, average,
								active_day, unique_author, popular_author, positive, negative, neutral, retweets,
								percent_retweets, total_urls, percent_urls,top_language, profile_twitter_attention
								,top_authority, top_authority_followers, total_leads ) 
							VALUES ( "'.date('Y-m-d H:i:s') .'",'. $array['source_id'] .','.$array['profile_id'] .','.$array['total'] .
								','. $array['average'] .',"'.$array['active_day'] .'",'.$array['unique_author'] 
								 .',"'.$array['popular_author'] .'",'. $array['positive_sentiment'] .','.$array['negative_sentiment'] .','.$array['neutral_sentiment'] 
								 .','.$array['retweets'].','. $array['percent_retweets'].','.$array['total_urls'].','.$array['percent_urls']
								 .',"'.$array['top_language'] .'",'. $array['profile_twitter_attention'] .',"'.$array['top_authority'] .'",'.$array['top_authority_followers'].','.$array['total_leads'] .') ' ;
...
* @ param array $item : the array of mentions
						( date , source_id , profile_id , author,label,
							 original_platform, sentiment, lead_action_name,
							  comments, removable, user_link_behavior,
							  user_followed, lead, message, message_link,      
							  user_id, username, user_profile, liked, like_status,
							   like_action, likes, shared, is_rich, mention_var,
							   tracked, reply_to, twitter_conversations_url, follow_status) 
...
*/
	...
	public function insertMention ($item) {
					...
					if (is_array($item['message_content']))
					{

						$item['message_content']=$item['message_content']['message'];
					}
						...
						$query ='INSERT INTO mentions 
							...
							VALUES ( "'.$item['date'] .'",'. $item['source_id'] .','.$item['profile_id'] 
								.',"'.$item['author'] .'","'.$item['label'] .'","'.$item['original_platform'] .
								'","'. $item['sentiment'] .'","'. mysqli_real_escape_string($this->con,$item['lead_action_name'])

								.'",'.$item['comments'].',"'.$item['removable'].'","'.$item['user_link_behavior']
								.'","'.$item['user_followed']
							
								 .'","'. $item['lead']
								 .'","'.mysqli_real_escape_string($this->con,$item['message_content']) .'","'. $item['message_link']
								 .'",'.$item['user_id'] .',"'. $item['user_name'] 
								 .'","'.$item['user_profile'] .'","'. $item['liked'] 
								 .'","'.$item['like_status'] .'","'. $item['like_action'] 
								 .'",'.$item['likes'] .',"'. $item['shared'] 
								 .'","'.$item['is_rich'] .'","'. $item['mention_var'] 
								 .'","'.$item['tracked'] .'","'. $item['reply_to'] 
								 .'","'.$item['twitter_conversations_url']
