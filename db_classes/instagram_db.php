...
*/
	...
	public function insertTrend ($trend, $sentiment) {
		...
		foreach ($trend as $item) {
				...
				$item['date']=explode("T",$item['date']); // get the date from datetime
...
* @ return array $result : the array of stats array   ( id, date , source_id , profile_id , total, average,
								max, min, top_day, positive, negative,neutral, top_influencer_by_impact, top_influencer_by_volume) 
*/
...
* @ param array $array : the array of stats  source_id , profile_id , total, average,
								max, min, top_day, positive, negative,neutral, 
								top_influencer_by_impact, top_influencer_by_volume) 
*
*
*/
	...
	public function insertStats ($array) {
		...
		$item['date']=explode(" ",date('Y-m-d H:i:s'));
				...
				if (is_null($raw['date'])==true) { 
						...
						$query ='INSERT INTO instagram_stats 
							...
							VALUES ( "'.date('Y-m-d H:i:s') .'",'. $array['source_id'] .','.$array['profile_id'] .','.$array['total'] .
								','. $array['average'] .','.$array['max'] .','.$array['min'] 
								 .',"'.$array['top_day'] .'",'. $array['positive_sentiment'] .','.$array['negative_sentiment']  
								 .','.$array['neutral_sentiment'].',"'. $array['top_influencer_by_impact'].'","'
								 .$array['top_influencer_by_volume'].'")';
...
* @ param array $item : the array of mentions
						 ( date , source_id , profile_id , username, message, link, negative, positive
                                ,comments, likes, author, original_platform) 
*
*
*/
    ...
    public function insertMention ($item) {
                        ...
                        $query ='INSERT INTO mentions 
                            ...
                            VALUES ( "'.$item['date'] .'",'. $item['source_id'] .','.$item['profile_id'] 
                            	...
                            	.',"'. mysqli_real_escape_string($this->con,$item['username']) 
                            	.'","'. mysqli_real_escape_string($this->con,$item['message']) 
                            	.'","'. mysqli_real_escape_string($this->con,$item['link']) .
                                '",'.$item['negative'] .
                                ','.$item['positive'] .
                                ','.$item['totalComments'] .
                                ','.$item['totalLikes'] .
                                ...
                                ',"'. $item['authorDisplayName'] .'","'. $item['original_platform'] .'") ' ;
