...
$resource_id=3; // facebook resource id
...
foreach ($getProfiles as $profile) {
	...
	$id=$profile['VH_id'];
	$URL=file_get_contents("https://app.viralheat.com/social/api/monitoring/pinterest/trend?days=1&profile_id=".$id."&api_key=".$api_key."&organization_id=".$organization_id);
	...
	if ($decode['error']==null) {
		...
		while ($i<$count) {
			...
			$post_count=$post_content['pins_count'];
		...
		// get the sentoment trend
		 ...
		 // insert the record
		$insertTrends=$pinterest_trend_dbClass->insertTrend($trend,$sentiment);
