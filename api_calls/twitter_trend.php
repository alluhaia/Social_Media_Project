...
$resource_id=2; // facebook resource id
...
foreach ($getProfiles as $profile) {
	...
	$id=$profile['VH_id'];
	$URL=file_get_contents("https://app.viralheat.com/social/api/monitoring/twitter/trend?days=1&profile_id=".$id."&api_key=".$api_key."&organization_id=".$organization_id);
	...
	if ($decode['error']==null) {
		...
		while ($i<$count) {
			...
			$post_count=$post_content['tweet_count'];
		...
		while ($i<$count) {
		 ...
		 // insert the record
		$insertTrends=$twitter_trend_dbClass->insertTrend($trend,$sentiment);
