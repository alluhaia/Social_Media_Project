...
$resource_id=7; // facebook resource id
...
foreach ($getProfiles as $profile) {
	...
	$id=$profile['VH_id'];
	$URL=file_get_contents("https://app.viralheat.com/social/api/monitoring/website/trend?days=1&profile_id=".$id."&api_key=".$api_key."&organization_id=".$organization_id);
	...
	if ($decode['error']==null) {
		...
		while ($i<$count) {
			...
			$post_count=$post_content['site_count'];
		...
		// // store into database
		$insertTrends=$website_trend_dbClass->insertTrend($trend,$sentiment);
...
print_r($website_trend_dbClass->gettrends (null, $resource_id, $id));
