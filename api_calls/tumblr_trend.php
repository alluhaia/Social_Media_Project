...
$resource_id=5; // facebook resource id
...
foreach ($getProfiles as $profile) {
	...
	$URL=file_get_contents("https://app.viralheat.com/social/api/monitoring/tumblr/trend?days=1&profile_id=".$id."&api_key=".$api_key."&organization_id=".$organization_id);
	...
	if ($decode['error']==null) {
		...
		while ($i<$count) {
			...
			$trend[]=array('source_id'=>$resource_id, 'profile_id'=>$id,'date'=>$date,'count'=>$post_count);
			$i++;
		...
		while ($i<$count) {
			...
			$sentiment[]=array('source_id'=>$resource_id, 'profile_id'=>$id, 'date'=>$sent_date,'positive'=>$positive,'negative'=>$negative, 'neutral'=>$neutral );
			$i++;
 		...
 		// insert the record
 		$insertTrends=$tumblr_trend_dbClass->insertTrend($trend,$sentiment);
